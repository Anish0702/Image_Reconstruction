...
# SAMPLING
# ========================
def random_sampling(img, ratio=0.15):
    h, w, _ = img.shape
    mask = np.zeros((h, w), dtype=bool)
    idx = np.random.choice(h*w, int(h*w*ratio), replace=False)
    mask.flat[idx] = True
    return mask
...
# SAVE IMAGES
# ========================
os.makedirs("Pictures\\Results\\Method 3\\results_dft_random", exist_ok=True)

imsave("Pictures\\Results\\Method 3\\results_dft_random/original.png", (img*255).astype(np.uint8))
imsave("Pictures\\Results\\Method 3\\results_dft_random/sampled.png", (sparse*255).astype(np.uint8))
imsave("Pictures\\Results\\Method 3\\results_dft_random/reconstructed.png", (recon*255).astype(np.uint8))
