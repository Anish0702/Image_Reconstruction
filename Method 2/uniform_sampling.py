...
def sample_pixels(image, segments, ratio=0.15):
    """Uniform Sampling
    Picks pixels at regular grid intervals.
    """
    h, w = image.shape[:2]
    mask = np.zeros((h, w), dtype=bool)

    step = int(np.sqrt(1 / ratio))  # spacing based on ratio
...
def run_pipeline(image_path, ratio=0.15):
    ...
    print("=== UNIFORM SAMPLING (CLUSTER-AWARE RECON) ===")
    ...
    plt.suptitle("Uniform Sampling + Cluster Reconstruction")
    plt.tight_layout()
    plt.savefig('Pictures\\Results\\Method 2\\results_uniform.png', dpi=300, bbox_inches='tight')
    plt.show()

if __name__ == '__main__':
    import skimage.data
    import os
    sample_path = "C:\\Users\\anish\\Downloads\\Image_Reconstruction\\Pictures\\Images\\kodim04.png"
