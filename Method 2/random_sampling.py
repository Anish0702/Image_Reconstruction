...
def sample_pixels(image, segments, ratio=0.2):
    ...
    Randomly select 20% of all pixels globally.
    """
    h, w = image.shape[:2]
...
def run_pipeline(image_path, ratio=0.2):
    ...
    print("=== RANDOM SAMPLING (CLUSTER-AWARE RECON) ===")
    ...
    plt.suptitle("Random Sampling + Cluster Reconstruction")
    plt.tight_layout()
    plt.savefig('Pictures\\Results\\Method 2\\results_random.png', dpi=300, bbox_inches='tight')
    plt.show()
